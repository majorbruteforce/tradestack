...
TEST_F(SideTreeTest, InsertsInNonEmptyTree)
{
    ...
    EXPECT_EQ(st->root, first) << "Root node should be node(key = " << first->key << ")";
    EXPECT_EQ(st->root->right, second)
            << "Right of root node should be node(key = " << second->key << ")";
    ...
    EXPECT_EQ(st->root, second) << "Root node should be node(key = " << second->key << ")";
    EXPECT_EQ(st->root->right, third)
            << "Right of root node should be node(key = " << third->key << ")";
    EXPECT_EQ(st->root->left, first)
            << "Right of root node should be node(key = " << first->key << ")";
}