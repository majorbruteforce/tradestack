...
struct Order {
  ...
  explicit Order(std::string id, std::string cid, std::uint64_t p,
                 std::uint64_t q, Side s, OrderType t)
      : id(std::move(id)),
...
inline void printOrder(const Order& ord, std::ostream& os = std::cout,
                       std::size_t width = 15) {
  ...
  utils::printField("Side", (ord.side == Side::Buy ? "Buy" : "Sell"), width,
                    os);
  utils::printField("Type", (ord.type == OrderType::Limit ? "Limit" : "Market"),
                    width, os);
}

inline std::ostream& operator<<(std::ostream& os, const Order& o) {
  return os << '{' << "id=" << o.id << ", cid=" << o.clientOrderId
            << ", price=" << o.price << ", rem=" << o.remainingQuantity
